...
???????????????????????????????????????????????????????????????
?                      appgui.py                              ?
?                  (DicomCreatorApp class)                    ?
???????????????????????????????????????????????????????????????
?                                                             ?
?  Remote Tab UI Components:                                 ?
?  - Preset dropdown (ttk.Combobox)                          ?
?  - Load/Save/Delete buttons (ttk.Button)                   ?
?                                                             ?
?  Event Handlers:                                            ?
?  - _on_preset_selected() - Combobox selection event        ?
?  - _load_preset() - Load button click                      ?
?  - _save_current_preset() - Save button click              ?
?  - _delete_preset() - Delete button click                  ?
